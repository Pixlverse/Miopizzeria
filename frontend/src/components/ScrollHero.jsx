  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (reduce) {
        ...
        gsap.set(rootRef.current.querySelectorAll(".ss-pizzaA"), { xPercent: 36 });
        gsap.set(rootRef.current.querySelectorAll(".ss-word1, .ss-copy"), { autoAlpha: 1 });
        return;
      }
      ...
      const ctx = gsap.context(() => {
        ...
        tl.to(".ss-copy", { autoAlpha: 0, x: -60, duration: 1.5 }, 2)
          .to(".ss-pizzaA", { xPercent: 0, scale: 1.18, rotate: -4, duration: 2 }, 2)
          .to(".ss-word1", { autoAlpha: 0, yPercent: -20, duration: 1.5 }, 2)
          .fromTo(".ss-word2", { autoAlpha: 0, yPercent: 22 }, { autoAlpha: 1, yPercent: 0, duration: 1.5 }, 3)
          .fromTo(".ss-info1", { autoAlpha: 0, y: 30 }, { autoAlpha: 1, y: 0, duration: 1.5 }, 3.3);
        ...
        tl.to(".ss-pizzaA", { autoAlpha: 0, scale: 1.35, duration: 1.5 }, 4.8)
          ...
          .fromTo(".ss-pizzaB", { autoAlpha: 0, scale: 0.9, rotate: 8 }, { autoAlpha: 1, scale: 1.18, rotate: -4, duration: 2 }, 5)
          .fromTo(".ss-word3", { autoAlpha: 0, yPercent: 22 }, { autoAlpha: 1, yPercent: 0, duration: 1.5 }, 5.4)
          .fromTo(".ss-info2", { autoAlpha: 0, y: 30 }, { autoAlpha: 1, y: 0, duration: 1.5 }, 5.7);
        ...
        tl.to(".ss-pizzaB", { scale: 0.85, yPercent: -6, duration: 1.5 }, 7.6)
          ...
          .fromTo(".ss-final", { autoAlpha: 0, y: 30 }, { autoAlpha: 1, y: 0, duration: 1.5 }, 7.9);
      }, rootRef);
    ...
    <section ref={rootRef} className="relative h-[380vh]">
      <div className="sticky top-0 flex h-[100svh] items-center overflow-hidden bg-rust">
        ...
        {[
          ...
          <div key={w.c} className={`${w.c} pointer-events-none absolute inset-0 grid place-items-center`}>
        ...
        <div className="pointer-events-none absolute inset-0 z-10 grid place-items-center">
          <div className="relative aspect-square w-[min(82vw,540px)]">
            <div className="ss-pizzaA absolute inset-0 drop-shadow-2xl">
              <Image src="/images/slider2.png" alt="Pepperoni Supreme pizza" fill priority sizes="(max-width: 768px) 82vw, 540px" className="object-contain" />
            </div>
            <div className="ss-pizzaB absolute inset-0 drop-shadow-2xl">
              <Image src="/images/slider1.png" alt="Garden Deluxe pizza" fill sizes="(max-width: 768px) 82vw, 540px" className="object-contain" />
            </div>
          </div>
        </div>

        {/* Scene 1 — hero copy */}
        <div className={`ss-copy absolute top-1/2 z-20 max-w-md -translate-y-1/2 pr-6 ${LEFT_INSET}`}>
          <div className="mb-4 flex gap-2">
            {["Classic", "Spicy", "Veggie"].map((f) => (
              <span key={f} className="rounded-full bg-cream/15 px-3 py-1 text-xs font-semibold text-cream ring-1 ring-cream/25">
                {f}
              </span>
            ))}
          </div>
        ...
        <div className={`ss-info1 absolute bottom-16 z-20 w-72 rounded-2xl bg-white/10 p-5 backdrop-blur-md ring-1 ring-white/20 ${LEFT_INSET}`}>
          <span className="inline-block rounded-full bg-cream px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-rust">Signature</span>
          <h3 className="mt-3 text-xl font-bold text-white">Pepperoni Supreme</h3>
          <p className="mt-1 text-sm text-cream/80">Molten mozzarella, double pepperoni, fire-kissed crust.</p>
          <div className="mt-4 flex items-center justify-between border-t border-white/15 pt-3">
            <span className="text-lg font-bold text-cream">QAR 45</span>
            <Link href="/#order" className="rounded-full bg-cream px-4 py-1.5 text-xs font-bold text-rust">Add to Cart</Link>
          </div>
        </div>

        {/* Scene 2 info */}
        <div className={`ss-info2 absolute bottom-16 z-20 w-72 rounded-2xl bg-white/10 p-5 backdrop-blur-md ring-1 ring-white/20 ${LEFT_INSET}`}>
          <span className="inline-block rounded-full bg-cream px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wide text-rust">Garden</span>
          <h3 className="mt-3 text-xl font-bold text-white">Garden Deluxe</h3>
          <p className="mt-1 text-sm text-cream/80">Olives, peppers, sweet corn and red onion on a hand-stretched base.</p>
          <div className="mt-4 flex items-center justify-between border-t border-white/15 pt-3">
        ...
        <div className="ss-final absolute inset-x-0 bottom-24 z-20 flex flex-col items-center text-center">
          <p className="font-display text-2xl italic text-cream/90">Craving more?</p>
          <Link
            href="/menu"
